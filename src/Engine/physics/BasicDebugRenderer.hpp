...
class BasicDebugRenderer : public JPH::DebugRendererSimple {
   public:
	...
	BasicDebugRenderer(fe::IRenderDevice* device) : device_(device) {
	...
	void Render(const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix) {
		...
		for (auto& pair : batches) {
			...
			if (batch.drawMode == 0) {
				device_->DrawDebugLines(reinterpret_cast<const float*>(batch.vertices.data()), static_cast<int>(batch.vertices.size()), viewMatrix, projectionMatrix);
			} else {
				device_->DrawDebugTriangles(reinterpret_cast<const float*>(batch.vertices.data()), static_cast<int>(batch.vertices.size()), viewMatrix, projectionMatrix);
			}
		}
   ...
   private:
	...
	void DrawSimpleString(RenderBatch& batch, const glm::vec3& pos, const std::string& text, const glm::vec4& color, float height) {
	...
	void DrawWireframeChar(RenderBatch& batch, const glm::vec3& pos, char c, const glm::vec4& color, float width, float height) {
	...
	void AddLine(RenderBatch& batch, const glm::vec3& from, const glm::vec3& to, const glm::vec4& color) {
		DebugVertex v1, v2;
		v1.position = from;
		v2.position = to;
		v1.color = color;
