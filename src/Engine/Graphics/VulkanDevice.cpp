...
void VulkanDevice::SetShaderPaths(VertexFormat format,  const std::string& vertPath,  const std::string& fragPath) {
 ...
 void VulkanDevice::Init(IWindow *window) {
...
void VulkanDevice::SubmitFrame() {
 ...
 void VulkanDevice::UnregisterWindow(const IWindow* window) {
...
void VulkanDevice::Clear() {
 ...
 VkDescriptorSet VulkanDevice::GetOrCreateTextureDescriptorSet(VkImageView imageView,  VkSampler sampler) {
 ...
 void VulkanDevice::createFrameDescriptorSets() {
 ...
 void VulkanDevice::DrawMesh(const IGPUBuffers* buffers,  const fe::IGPUTexture* texture) {
...
VkPipeline requiredPipeline = GetGraphicsPipeline(vkBuffers->vertexFormat, transparentMode_, reverseWinding_);
 ...
 void VulkanDevice::DrawIndirect(VkBuffer vertexBuffer,  VkBuffer indexBuffer, 
 ...
 void VulkanDevice::UpdateIndirectDescriptorSet(VkDescriptorSet set,  VkImageView textureView,  VkSampler sampler) {
 ...
 std::unique_ptr<IGPUBuffers> VulkanDevice::CreateGPUBuffers() {
 ...
 std::unique_ptr<fe::IGPUTexture> VulkanDevice::CreateGPUTexture() {

		return std::make_unique<fe::VulkanGPUTexture>();
}

void VulkanDevice::UploadBuffers(IGPUBuffers* buffers,
 ...
 void VulkanDevice::UploadTexture(fe::IGPUTexture* texture, 
  const std::string& path,  fe::TextureScaling scaling) {
 ...
 void VulkanDevice::UploadTexture(fe::IGPUTexture* texture, 
  const fe::ImageData& image,  fe::TextureScaling scaling) {
 ...
 void VulkanDevice::UploadTextureArray(fe::IGPUTexture* texture, 
  const std::vector<std::string>& paths,  fe::TextureScaling scaling) {
 ...
 size_t VulkanDevice::GetWindowCount() const {
 ...
 VkImage VulkanDevice::GetColorAttachmentImage(uint64_t handle) const {
 ...
 VkDescriptorPool VulkanDevice::GetDescriptorPool() const {
 ...
 size_t VulkanDevice::GetSwapChainImageCount() const {
...
VkPipeline VulkanDevice::GetGraphicsPipeline(VertexFormat format, bool transparent, bool cw) const {
 		switch (format) {
 			case VertexFormat::Standard:
 				return transparent ? (cw ? pipelineSets_[0].cwTransparent : pipelineSets_[0].transparent)
 				                   : (cw ? pipelineSets_[0].cw : pipelineSets_[0].normal);
 			case VertexFormat::Array:
 				return transparent ? (cw ? pipelineSets_[1].cwTransparent : pipelineSets_[1].transparent)
 				                   : (cw ? pipelineSets_[1].cw : pipelineSets_[1].normal);
 			case VertexFormat::Packed:
 			default:
 				return transparent ? (cw ? pipelineSets_[2].cwTransparent : pipelineSets_[2].transparent)
 				                   : (cw ? pipelineSets_[2].cw : pipelineSets_[2].normal);
 		}
 	
 }


 const glm::mat4& VulkanDevice::GetViewMatrix() const {
 return currentView_; 
 }


 const glm::mat4& VulkanDevice::GetProjectionMatrix() const {
 return currentProj_; 
 }



 void VulkanDevice::SetPreferIntegratedGPU(bool v) {
 preferIntegratedGPU_ = v; 
}


 VkPipelineLayout VulkanDevice::GetPipelineLayout() const {
 return pipelineLayout_; 
}


 VkDescriptorSetLayout VulkanDevice::GetDescriptorSetLayout() const {
 ...
 void VulkanDevice::SetModelMatrix(const glm::mat4& m) {
 currentModel_ = m; updateUniformBuffer(currentFrame_); 
}


 void VulkanDevice::SetViewMatrix(const glm::mat4& v) {
 currentView_ = v; 
}


 void VulkanDevice::SetProjectionMatrix(const glm::mat4& p) {
 currentProj_ = p; 
}



 void VulkanDevice::SetMat4(const char* name,  const glm::mat4& value) {
 ...
 void VulkanDevice::SetVec3(const char* name,  const glm::vec3& value) {
 ...
 void VulkanDevice::SetFrontFace(bool ccw) {
 ...
 void VulkanDevice::SetTransparentMode(bool enabled) {
 ...
 bool VulkanDevice::ReadDepthBuffer(std::vector<float>& outDepths,  int& outW,  int& outH) {
 ...
 void* VulkanDevice::UploadToImGui(const unsigned char* rgba,  int w,  int h) {
 ...
 uint64_t VulkanDevice::CreateFramebuffer(uint64_t nativeImage,  uint32_t w,  uint32_t h,  uint32_t layer,  uint64_t depthFormat,  uint64_t colorFormat) {
 ...
 void VulkanDevice::DestroyFramebuffer(uint64_t fb) {
 ...
 void VulkanDevice::BeginVRFrame() {
 ...
 void VulkanDevice::BeginEyeFrame(uint64_t fb,  uint32_t w,  uint32_t h) {
...
vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, GetGraphicsPipeline(VertexFormat::Standard));
 ...
 void VulkanDevice::BeginExternalFrame(uint64_t fb,  uint32_t w,  uint32_t h) {
 ...
 void VulkanDevice::EndExternalFrame() {
 ...
 bool VulkanDevice::IsVulkan() const {
 return true; 
}



 void VulkanDevice::SetActiveWindow(IWindow* window) {

}



 uint64_t VulkanDevice::CreateColorAttachment(uint32_t w,  uint32_t h) {
 ...
 void VulkanDevice::DestroyColorAttachment(uint64_t handle) {
 ...
 uint64_t VulkanDevice::GetSwapchainFormat() const {
...
void VulkanDevice::setupWindowResources(IWindow* window) {
 ...
 void VulkanDevice::cleanupWindowResources(VulkanWindowResources& res) {
 ...
 VkRenderPass VulkanDevice::getOrCreateXrRenderPass(VkFormat colorFormat,  VkFormat depthFormat) {
 ...
 QueueFamilyIndices VulkanDevice::findQueueFamilies(VkPhysicalDevice dev,  VkSurfaceKHR surface) {
 ...
 SwapChainSupportDetails VulkanDevice::querySwapChainSupport(VkPhysicalDevice dev,  VkSurfaceKHR surface) {
 ...
 void VulkanDevice::createSwapChain(IWindow *window) {
 ...
 std::vector<const IWindow*> VulkanDevice::GetWindows() {
 ...
 VkSwapchainKHR VulkanDevice::createSwapChain(VkSurfaceKHR surface,  VulkanWindowResources& res) {
 ...
 VkPresentModeKHR VulkanDevice::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& modes,  bool vsync) {
 ...
 void VulkanDevice::createImageViews(VulkanWindowResources& res) {
 ...
 // Shader helpers
 // ---------------------------------------------------------------
 std::vector<char> VulkanDevice::readFile(const std::string& path) {
 ...
 void VulkanDevice::createGraphicsPipeline(const std::string& vertPath, const std::string& fragPath,
		...
		} else if (format == VertexFormat::Packed) {
			bindingDesc.binding = 0;
 ...
 // Depth resources
 // ---------------------------------------------------------------
 void VulkanDevice::createDepthResources(VulkanWindowResources& res) {

		VkFormat depthFormat = findDepthFormat();
		createImage(res.extent.width, res.extent.height, depthFormat,
			VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
...
void VulkanDevice::createImage(uint32_t width,  uint32_t height,  VkFormat format, 
     ...
     VkImage& image,  VkDeviceMemory& imageMemory) {
		...
		fe::vk::CreateImage(_device, _physicalDevice, width, height, format, tiling, usage, properties, image, imageMemory);
...
VkImageView VulkanDevice::createImageView(VkImage image,  VkFormat format,  VkImageAspectFlags aspectFlags) {
		...
		return fe::vk::CreateImageView(_device, image, format, aspectFlags);
 ...
 void VulkanDevice::createFramebuffers(VulkanWindowResources& res) {
 ...
 void VulkanDevice::createCommandBuffers(VulkanWindowResources& res) {
...
uint32_t VulkanDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
		...
		return fe::vk::FindMemoryType(_physicalDevice, typeFilter, properties);
...
void VulkanDevice::createBuffer(VkDeviceSize size,  VkBufferUsageFlags usage, 
     ...
     VkBuffer& buffer,  VkDeviceMemory& bufferMemory) {
		...
		fe::vk::CreateBuffer(_device, _physicalDevice, size, usage, properties, buffer, bufferMemory);
...
void VulkanDevice::copyBuffer(VkBuffer srcBuffer,  VkBuffer dstBuffer,  VkDeviceSize size) {
		...
		fe::vk::CopyBuffer(_device, commandPool_, graphicsQueue_, srcBuffer, dstBuffer, size);
...
// ---------------------------------------------------------------
 void VulkanDevice::createDefaultTexture() {
...
void VulkanDevice::transitionImageLayout(VkImage image,  VkFormat format,  VkImageLayout oldLayout,  VkImageLayout newLayout) {
		...
		(void)format;
		fe::vk::TransitionImageLayout(_device, commandPool_, graphicsQueue_, image, oldLayout, newLayout);
...
void VulkanDevice::copyBufferToImage(VkBuffer buffer,  VkImage image,  uint32_t width,  uint32_t height) {
		...
		fe::vk::CopyBufferToImage(_device, commandPool_, graphicsQueue_, buffer, image, width, height);
 ...
 void VulkanDevice::createPerWindowSemaphores(VulkanWindowResources& res) {
 ...
 void VulkanDevice::cleanupSwapChain(VulkanWindowResources& res) {
 ...
 void VulkanDevice::recreateSwapChain(const IWindow *window) {
 ...
 void VulkanDevice::cleanup() {
		...
		windowRegistry.clear();
 		...
 		for (auto& pipelineSet : pipelineSets_) {
 			vkDestroyPipeline(_device, pipelineSet.normal, nullptr);
 			vkDestroyPipeline(_device, pipelineSet.cw, nullptr);
 			vkDestroyPipeline(_device, pipelineSet.transparent, nullptr);
 			vkDestroyPipeline(_device, pipelineSet.cwTransparent, nullptr);
 		}
		if (pipelineLayout_ != VK_NULL_HANDLE)
