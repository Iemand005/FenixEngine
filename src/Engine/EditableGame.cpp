...
// BasicDebugRenderer::DrawImGuiToggle("Show physics debug");
	...
	ImGui::Begin("Joysticks");
	{
		...
		static std::vector<float> rumbleLow, rumbleHigh, rumbleDurations;
		constForceLevels.resize(joysticks.size());
		shakeMagnitudes.resize(joysticks.size());
		springCoeffs.resize(joysticks.size());
		rumbleLow.resize(joysticks.size(), 0.5f);
		rumbleHigh.resize(joysticks.size(), 0.5f);
		...
		for (size_t i = 0; i < joysticks.size(); ++i) {
			...
			ImGui::Indent();
			ImGui::SliderFloat(("Big Motor##rumble" + std::to_string(i)).c_str(), &rumbleLow[i], 0.0f, 1.0f);
			ImGui::SliderFloat(("Small Motor##rumble" + std::to_string(i)).c_str(), &rumbleHigh[i], 0.0f, 1.0f);
			...
			if (ImGui::Button(("Rumble##" + std::to_string(i)).c_str())) {
				joysticks[i].Rumble(rumbleLow[i], rumbleHigh[i], static_cast<Uint32>(rumbleDurations[i]));
			}
